...
contract ArtToken is ERC721, Ownable {
    ...
    constructor(
        ...
        uint8 _devPercantage
    ) public ERC721("Art Token", "AT") {
     ...
     */
    function uploadToMarketPlace(uint256 _price, string memory _name)
        public
        payable
        returns (uint256)
    {
        require(
            msg.value == _price.div(devPercantage) + _price.div(holderPercantage),
            "uploadToMarketPlace: Price does not sum up"
        );
        ...
        public
    {
        require(
            msg.sender == marketPlaceAddress,
            "updateSellerAddress: Call not from marketPlace"
        );
    ...
    function splitBalance(
        ...
        if (isBought) {
            uint256 creatorShare = price.div(creatorPercantage);
    ...
    function reflectDividend(uint256 price) private {
