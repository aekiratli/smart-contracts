...
// File @openzeppelin/contracts/utils/introspection/IERC165.sol@v4.0.0-rc.0
...
// File @openzeppelin/contracts/utils/Counters.sol@v4.0.0-rc.0
...
// File contracts/PastaNft.sol

pragma solidity ^0.8.0;


contract PastaNft is Ownable,  ERC721Enumerable{
  using Counters for Counters.Counter;
  Counters.Counter public _tokenIdTracker;

  string private _baseTokenURI;
  uint256 private _price = 1 ether;
  uint private _max = 20;
  uint public _reservedForGiveaway = 10;
  address private _walletAddress = 0x55CfFB31c96f9FA503cdDbC1AbfE9D094904ed71;
  ...
  constructor (string memory baseTokenURI) ERC721("PastaNft", "PNFT") {
  ...
  function mint(uint amount) public payable {
    ...
    require(_tokenIdTracker.current() + amount <= _max - _reservedForGiveaway, "not enough nft left to mint amount");
    ...
    function giveAwayNft(address _to, uint256 _amount) external onlyOwner() {
        require( _amount <= _reservedForGiveaway, "Exceeds reserved supply" );

        for(uint256 i; i < _amount; i++){
            _mint( _to, _tokenIdTracker.current());
            minter[_tokenIdTracker.current()] = _to; // to be tested

            _tokenIdTracker.increment();

        }

        _reservedForGiveaway -= _amount;
    }
  ...
  function splitBalance(uint256 amount) private {
      ...
      payable(_walletAddress).transfer(mintingShare1);
  }
